...
import { canvasAppearanceForTheme, DEFAULT_CANVAS_BACKGROUND_MODE, normalizeCanvasAppearance, resolveCanvasAppearance, type CanvasAppearance } from "@/lib/canvas/canvas-appearance";
...
export function useReadOnlyCanvas(project: CanvasProject | null) {
    const colorTheme = useCanvasThemeStore((state) => state.theme);
    const [nodes, setNodes] = useState<CanvasNodeData[]>([]);
    const [connections, setConnections] = useState<CanvasProject["connections"]>([]);
    ...
    useEffect(() => {
        ...
        const nextAppearance = project.appearance ? normalizeCanvasAppearance(project.appearance, colorTheme) : canvasAppearanceForTheme(colorTheme);
    ...
    return { nodes, setNodes, connections, appearance, backgroundMode, viewport, setViewport, viewportRef };
}
...
export function ReadOnlyCanvasView({
    ...
    const { nodes, setNodes, connections, appearance, backgroundMode, viewport, setViewport, viewportRef } = useReadOnlyCanvas(project);
    const [selectedNodeId, setSelectedNodeId] = useState<string | null>(null);
    const [infoNodeId, setInfoNodeId] = useState<string | null>(null);
    const [dragOffset, setDragOffset] = useState<Position | null>(null);
    const [contextMenu, setContextMenu] = useState<ContextMenu | null>(null);
    const tour = mode === "tour";
    const unauthorized = useCallback(() => message.warning(tour ? "只读模式，如需创建请点击「复制项目」" : "未授权：分享画布仅供查看，该操作不会执行。"), [message, tour]);
    ...
    const renderSharedNode = useCallback((node: CanvasNodeData): ReactNode => {
        if (node.type === CanvasNodeType.Script) return <SharedScriptNode node={node} onUnauthorized={unauthorized} hideGenerate={tour} />;
        ...
        return <SharedConfigNode node={node} onUnauthorized={unauthorized} hideGenerate={tour} />;
    }, [connections, nodes, theme, unauthorized, tour]);
    const toolbarNode = selectedNodeId ? nodeById.get(selectedNodeId) || null : null;
    ...
    return (
        <main className="relative h-screen overflow-hidden" style={{ background: resolveCanvasAppearance(appearance, colorTheme).background, color: theme.node.text }}>
            <header className="pointer-events-none absolute inset-x-0 top-0 z-[var(--z-panel-floating)] flex h-16 items-center justify-between px-5">
                <div className="pointer-events-auto flex min-w-0 items-center gap-3">
                    ...
                    <span className="inline-flex items-center gap-1 text-xs" style={{ color: theme.node.muted }}>
                        ...
                        {tour ? "只读模式，如需创建请点击「复制项目」" : "只读分享"}
                    </span>
                </div>
                <div className="pointer-events-auto flex items-center gap-2">
                    {tour && onCopy ? <Button type="primary" onClick={onCopy}>复制项目</Button> : null}
                    {headerRight}
                </div>
            </header>
            ...
            <div className="pointer-events-none absolute bottom-5 right-5 z-[var(--z-panel-floating)] max-w-[340px] text-right text-xs leading-5" style={{ color: theme.node.muted }}>{tour ? "只读参观，节点图不会写回作者画布" : "访客操作仅在当前页面临时生效"}</div>

            {contextMenu ? <SharedContextMenu menu={contextMenu} tour={tour} onAdd={addNode} onInfo={() => { if (contextMenu.nodeId) setInfoNodeId(contextMenu.nodeId); setContextMenu(null); }} onUnauthorized={() => { setContextMenu(null); unauthorized(); }} /> : null}
            <CanvasNodeInfoModal node={infoNode} open={Boolean(infoNode)} onClose={() => setInfoNodeId(null)} readOnly onUnauthorized={unauthorized} />
        </main>
    );
}

function SharedContextMenu({ menu, tour, onAdd, onInfo, onUnauthorized }: { menu: ContextMenu; tour: boolean; onAdd: (type: CanvasNodeType) => void; onInfo: () => void; onUnauthorized: () => void }) {
    ...
    return <div data-canvas-no-zoom className="absolute z-[var(--z-modal)] min-w-48 rounded-lg border p-1.5 shadow-xl" style={{ left: menu.x, top: menu.y, background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
        {menu.nodeId ? <><MenuButton icon={<Eye />} label="查看节点信息" onClick={onInfo} />{tour ? null : <MenuButton icon={<LockKeyhole />} label="编辑或生成" onClick={onUnauthorized} />}</> : tour ? <div className="px-2 py-1.5 text-xs" style={{ color: theme.node.muted }}>只读参观</div> : <>
...
function SharedConfigNode({ node, onUnauthorized, hideGenerate }: { node: CanvasNodeData; onUnauthorized: () => void; hideGenerate?: boolean }) {
    const theme = canvasThemes[useCanvasThemeStore((state) => state.theme)];
    return <div className="flex h-full w-full flex-col overflow-hidden rounded-[var(--panel-radius)]">
        ...
        {hideGenerate ? null : <div className="flex h-12 shrink-0 items-center justify-end border-t px-3" style={{ borderColor: theme.node.stroke }}><Button size="small" icon={<Send className="size-3.5" />} onMouseDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); onUnauthorized(); }}>生成</Button></div>}
    </div>;
}

function SharedScriptNode({ node, onUnauthorized, hideGenerate }: { node: CanvasNodeData; onUnauthorized: () => void; hideGenerate?: boolean }) {
    const theme = canvasThemes[useCanvasThemeStore((state) => state.theme)];
    const rows = node.metadata?.storyboard?.rows || [];
    return <div className="flex h-full w-full flex-col overflow-hidden rounded-[var(--panel-radius)]">
        <div className="flex h-10 shrink-0 items-center gap-2 border-b px-4" style={{ background: theme.node.panel, borderColor: theme.node.stroke }}><Clapperboard className="size-4" /><span className="min-w-0 flex-1 truncate text-sm font-semibold">{node.title}</span><span className="text-xs" style={{ color: theme.node.muted }}>{rows.length} 镜</span>{hideGenerate ? null : <button type="button" className="grid size-7 place-items-center rounded hover:bg-black/5 dark:hover:bg-white/10" onMouseDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); onUnauthorized(); }} aria-label="一键创建视频节点"><Video className="size-3.5" /></button>}</div>
...
export function ReadOnlyCanvasError({ title, description }: { title: string; description: string }) {
    return <div className="grid h-screen place-items-center px-5"><WorkspaceStateFallback title={title} description={description} /></div>;
}

function WorkspaceStateFallback({ title, description }: { title: string; description: string }) {
    return (
        <div className="max-w-md space-y-4 text-center">
            <h1 className="text-xl font-semibold">{title}</h1>
            <p className="text-sm text-foreground/60">{description}</p>
            <Link to="/plaza"><Button>返回作品广场</Button></Link>
        </div>
    );
}
